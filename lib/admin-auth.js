...
export async function loadAdminSettings({ force = false } = {}) {
  ...
  _settingsCachePromise = (async () => {
    try {
      ...
      // TEMP DIAGNOSTIC: log row shape, never the values. The hash_fingerprint
      // is a non-reversible 8-char hash of the bcrypt hash, so we can compare
      // across calls to see if the value actually changed without ever
      // printing the secret.
      if (r.data && Array.isArray(r.data) && r.data[0]) {
        const row = r.data[0];
        const val = row.value;
        const fingerprint = val && typeof val.password_hash === 'string'
          ? crypto.createHash('sha256').update(val.password_hash).digest('hex').slice(0, 8)
          : null;
        console.error('[loadAdminSettings] row shape:', JSON.stringify({
          status: r.status,
          rowKeys: Object.keys(row),
          valueKeys: val && typeof val === 'object' ? Object.keys(val) : null,
          hashLen: val && typeof val.password_hash === 'string' ? val.password_hash.length : null,
          secretLen: val && typeof val.session_secret === 'string' ? val.session_secret.length : null,
          hashFp: fingerprint,
        }));
      } else {
        console.error('[loadAdminSettings] unexpected response:', JSON.stringify({
          status: r.status,
          ok: r.ok,
          dataType: typeof r.data,
          isArray: Array.isArray(r.data),
          rawLen: r.raw ? r.raw.length : 0,
        }));
      }
...
export async function writeAdminSettings({ passwordHash, sessionSecret }) {
  ...
  // TEMP DIAGNOSTIC: log only the response shape, never the body. The
  // hash_fp is a non-reversible fingerprint of whatever came back from
  // the PATCH (with Prefer: return=representation). Comparing it to the
  // hash_fp printed by loadAdminSettings immediately after tells us
  // whether the row was actually updated. We deliberately do NOT log
  // r.raw — the representation contains the full hash and secret.
  let writtenHashFp = null;
  if (r.data && Array.isArray(r.data) && r.data[0] && r.data[0].value && r.data[0].value.password_hash) {
    writtenHashFp = crypto.createHash('sha256').update(r.data[0].value.password_hash).digest('hex').slice(0, 8);
  }
  console.error('[writeAdminSettings] PATCH result:', JSON.stringify({
    status: r.status,
    ok: r.ok,
    bodyRows: r.data && Array.isArray(r.data) ? r.data.length : 0,
    bodySent: { valueKeys: Object.keys(value), updated_at: 'redacted' },
    writtenHashFp,
  }));
  if (r.ok) {
    invalidateSettings();
